...
class MyMainWindow(QWidget, Ui_Form):
    ...
    def identifyID(self):
        ...
        # print("卡号是：", self.ID)
        # "SELECT image FROM image LIMIT 1"
        try:
...
# class WorkTheard(QThread):
#     _sinagal = pyqtSignal()  # 多线程信号
#
#     def __init__(self, *sin, parent=None):
#         super(WorkTheard, self).__init__(parent)
#         self.read_info = None
#         self.open_info = None
#         self.sin = sin
#
...
#         if self.sin == 1:
#             fb = open("../faceID/img2.jpg", 'rb')  # 打开文件
#             self.read_info = fb.read()  # 二进制读取人脸信息
#             fb.close()  # 关闭链接
#             self._sinout.emit()
#         else:
#             image = self.helper.readImage("select informaTable from stu_info limit 1")
